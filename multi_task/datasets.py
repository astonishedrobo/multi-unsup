...
import presets
from loaders.unsup_loader import NYUDepth, NYUSeg


def get_transform(train):
    if train:
        return presets.SegmentationPresetTrain(base_size=32, crop_size=32)
    
    else:
        return presets.SegmentationPresetVal(base_size=32)
...
def get_dataset(params, configs):
    ...
    if 'celeba' in params['dataset']:
        ...
        train_dst = NYUSeg(configs[dataset]['train']['image_paths'], configs[dataset]['train']['target_paths'], get_transform(True), False, params)
        val_dst = NYUSeg(configs[dataset]['valid']['image_paths'], configs[dataset]['valid']['target_paths'], get_transform(False), False, params)
